  ...
  useEffect(() => {
    const fetchByToken = async () => {
      try {
        ...
        if (!token) { setState('error'); setErrorMsg('Anda harus login sebagai Admin.'); return; }

        const res = await fetch(`${API_URL}/api/admin/checkin/${qrToken}`, {
          headers: { Authorization: `Bearer ${token}`, 'x-maker-key': MAKER_KEY },
        });
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/admin/reservasi/${reservasi.id}/check-in`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}`, 'x-maker-key': MAKER_KEY, 'Content-Type': 'application/json' },
      });
      const json = await res.json();
